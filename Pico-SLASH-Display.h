...
#ifndef __PICO_ASTL_DISPLAY_H
#define __PICO_ASTL_DISPLAY_H
...
#define MAX_PARSE_UNITS   10
#define MAX_PARSE_LENGTH  40

#define AIRCR_Register (*((volatile UINT32 *)(PPB_BASE + 0x0ED0C)))  // used to "software-reset" core0 (we can't use it since we use both cores).
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------- *\
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------- *\
                                                                         Callback functions ID.
\* ------------------------------------------------------------------------------------------------------------------------------------------------------------- */
#define CALLBACK_NONE    0
#define CALLBACK_1MS     1
#define CALLBACK_50MS    2
#define CALLBACK_1000MS  3
#define CALLBACK_ISR     4
#define CALLBACK_DISPLAY 5  // core1

#define MAX_CALLBACK_DURATION_UNITS   500
...
#define DEBUG_INDICATORS  0x0000000000001000  // debug indicators on LED matrix.
#define DEBUG_IR          0x0000000000002000  // debug infrared remote control reception and decoding.
#define DEBUG_MATRIX      0x0000000000004000  // debug matrix display operations.
#define DEBUG_MQTT        0x0000000000008000  // debug Network Time Protocol.
#define DEBUG_NTP         0x0000000000010000  // debug Network Time Protocol.
#define DEBUG_PWM         0x0000000000020000  // debug PWM behavior.
#define DEBUG_REMINDER    0x0000000000040000  // debug reminder mechanism.
#define DEBUG_SCROLL      0x0000000000080000  // debug scroll mechanism.
#define DEBUG_SERVICE     0x0000000000100000  // debug service light algorithm.
#define DEBUG_SOUND_QUEUE 0x0000000000200000  // debug sound queue engine.
#define DEBUG_STARTUP     0x0000000000400000  // debug startup sequence.
#define DEBUG_DST_TIME    0x0000000000800000  // debug Daylight Saving Time related logic.
#define DEBUG_TEST        0x0000000001000000  // debug test section.
#define DEBUG_WATCHDOG    0x0000000002000000  // debug watchdog behavior.
#define DEBUG_WIFI        0x0000000004000000  // debug WiFi communications.
#define DEBUG_WINDOW      0x0000000008000000  // debug window algorithm.
...
{
  UINT16 LineNumber;
  UCHAR  FunctionName[32];
  UCHAR  Date[15];
  UCHAR  Time[15];
  UCHAR  DebugText[64];
};

#define MAX_MEM_DEBUG     500
...
#ifdef OLD_BOARD  // for those who have the old RGB-Matrix control board hardware version (two Pico-type sockets side-by-side).
/* Make sure the range remains 999 to keep the ratio the same with the brightness settings. */
#define PWM_LO_LIMIT          1001  //  lowest possible value for PWM Level (highest display brightness).
#define PWM_HI_LIMIT          2000  // highest possible value for PWM Level  (lowest display brightness).
#else  // OLD_BOARD
/* Make sure the range remains 999 to keep the ratio the same with the brightness settings. */
#define PWM_LO_LIMIT           994  //  lowest possible value for PWM Level (highest display brightness).
#define PWM_HI_LIMIT          1993  // highest possible value for PWM Level  (lowest display brightness).
#endif  // OLD_BOARD


#define BRIGHTNESS_LO_LIMIT      1  // lowest valid brightness value (barely visible LED display).
#define BRIGHTNESS_HI_LIMIT   1000  // highest brightness for LED display.

#define LIGHT_LO_LIMIT          60  // this value is considered to be the  lowest light reading value (although it could theorically go down to 0).
#define LIGHT_HI_LIMIT        4000  // this value is considered to be the highest light reading value (although it could theorically go up to 4096).
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------- *\
                                                                             Color definitions.
\* ------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* Those values are dependant of hardware wiring and must not be changed. */
#define BLACK      0x00
#define BLUE       0x01
#define GREEN      0x02
#define CYAN       0x03
...
struct active_alarm
{
  ...
  UINT64 PreviousTimer;    // timer value of this alarm's last ring (must not be in Alarm structure saved in flash).
};
...
#define MAX_REMINDERS1    20  // number of reminders of type 1.
...
#define MAX_FLASH_SECTORS          2  // maximum number of flash memory sectors used to store device configuration.
#define FLASH_FIRST_SECTOR  0x1FF000  // start address of first flash memory sector used to store device configuration (counting backward).

struct config0
{
  UCHAR  Version[8];               // firmware version number (format: "100.00a" - and including end-of-string).
  UINT16 FootPrint;                // to let us know that flash has already been initialized before.
  ...
  INT16  DeltaTime;                // (in minutes) value to add to UTC time (Universal Time Coordinate) to get the local time.
  ...
  UINT32 TotalWiFiErrors;          // cumulative number of WiFi connection errors.
  UINT32 TotalNTPErrors;           // cumulative number of NTP errors (including when WiFi is in problem).
  UINT32 TotalMQTTErrors;          // cumulative number of MQTT errors (including when MQTT is in problem).
  UINT32 TotalZigBeeErrors;        // cumulative number of ZigBee errors.
  UCHAR  NetworkName[40];          // network name (SSID) for Wi-Fi network. Note: SSID begins at position 5 of the variable string, so that a "footprint" can be confirmed prior to writing to flash.
  UCHAR  NetworkPassword[72];      // password for Wi-Fi network. Note: password begins at position 5 of the variable string, for the same reason as SSID above.
  ip_addr_t MQTTBrokerAddress;     // IP address of MQTT broker.
  UCHAR  MQTTPassword[40];         // password for MQTT broker.
  ...
  UINT8  Reserved[106];            // reserve the rest of this flash sector space for future use.
...
struct config1
{
  ...
  UINT8  Reserved[2156];           // reserve the rest of this flash sector space for future use.
  UINT16 Crc16;                    // crc16 of all data above to validate configuration.
}Config1;
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------- *\
...
#define MAX_MESSAGE_LENGTH   2048  // total length of all ASCII messages queued for scrolling.
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------- *\
                                                              Alternate display related definitions.
\* ------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* Maximum number of active alternative displays. */
#define MAX_ALTERNATES 20

/* Specific alternates positions. */
#define ALT_POS_INS_TEMP    0  // inside temperature.
#define ALT_POS_OUT_TEMP    1  // outside temperature.
#define ALT_POS_HUMIDITY    2  // outside humidity.
#define ALT_POS_HUMIDEX     3  // outside humidex factor.
#define ALT_POS_PRESSURE    4  // outside atmospheric pressure.
#define ALT_POS_OUT_LIGHT   5  // outside ambient light level.
#define ALT_POS_OUT_PROX    6  // outside proximity sensor value (LTR559).

/* Alternate display types. */
#define ALT_INFO     0  // Alternate for internal information only, not to be displayed.
#define ALT_DISPLAY  1  // Alternate for information, to be displayed in alternance (temperature, humidity, etc).
#define ALT_ALARM    2  // Alternate generated by an alarm, to be displayed in priority.

struct alternate
{
  UINT16 StartLine;  // matrix line where to display.
  UINT8  Type;       // alternate type.
  UCHAR  Text[32];   // complete string to be displayed.
  UINT32 Duration;   // validity duration of this item (in msec).
  UINT64 SeedTime;   // timer value when validity begins.
};
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------- *\
                                                           End if alternate display related definitions.
\* ------------------------------------------------------------------------------------------------------------------------------------------------------------- */
...
#define WIN_ALL          7
#define WIN_TEST         8
#define WIN_FUNCTION     9
#define WIN_DICE        10
#define WIN_LIGHT       11
#define WIN_MESSAGE     12
#define WIN_SETUP       13
#define WIN_HI_LIMIT    14  // must be one more than last window defined.
...
struct window
{
  ...
  UINT8  LastBoxState;         // defined what to do with last border after exploding the window (ACTION_DRAW or ACTION_ERASE).
  ...
  UINT64 BlinkOnTimer;         // indicate timer last time blinking pixels were turned On.
  INT16  CountDown;            // if not null, indicating a count-down time (in seconds) to clear before restoring back link window(s).
